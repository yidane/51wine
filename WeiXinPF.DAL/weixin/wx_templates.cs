...
namespace WeiXinPF.DAL
{
	...
	public partial class wx_templates
	{
		...
		public WeiXinPF.Model.wx_templates GetModel(int id)
		{
			...
			WeiXinPF.Model.wx_templates model=new WeiXinPF.Model.wx_templates();
		...
		/// 得到一个对象实体
		/// </summary>
		public WeiXinPF.Model.wx_templates DataRowToModel(DataRow row)
		{
			WeiXinPF.Model.wx_templates model=new WeiXinPF.Model.wx_templates();
			if (row != null)
			{
        ...
        public string GetLieBiaoTemplatesFileNameByWid(int wid)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,tFileName from wx_templates ");
            strSql.Append(" where id=(select listTid from wx_templates_wcode where wid=@wid)");
            SqlParameter[] parameters = {
					new SqlParameter("@wid", SqlDbType.Int,4)
			};
            parameters[0].Value = wid;
        ...
        public string GetDetailTemplatesFileNameByWid(int wid)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,tFileName from wx_templates ");
            strSql.Append(" where id=(select detailTid from wx_templates_wcode where wid=@wid)");
            SqlParameter[] parameters = {
					new SqlParameter("@wid", SqlDbType.Int,4)
			};
            parameters[0].Value = wid;
