...
namespace Travel.Infrastructure.WeiXin.Extra
{
    public class HttpHelper
    {
        ...
        public static Stream Get(string url, IEnumerable<KeyValuePair<string, object>> data = null, CookieContainer cc = null, string refer = null)
        {
            return Get(url, FormatData(data), cc, refer);
        }

        public static T Get<T>(string url, IEnumerable<KeyValuePair<string, object>> data = null, CookieContainer cc = null, string refer = null)
        {
            try
            {
                var s = FormatData(data);
                return JsonConvert.DeserializeObject<T>(new StreamReader(Get(url, s, cc, refer), DefaultEncoding).ReadToEnd());
            }
            catch
            {
                return default(T);
        ...
        public static string GetString(string url, string queryString = null, CookieContainer cc = null, string refer = null)
        {
            return new StreamReader(Get(url, queryString, cc, refer)).ReadToEnd();
        }

        public static string GetString(string url, IEnumerable<KeyValuePair<string, object>> data = null, CookieContainer cc = null, string refer = null)
        {
        ...
        public static T Post<T>(string url, IEnumerable<KeyValuePair<string, object>> data = null, CookieContainer cc = null, string refer = null)
        {
            var s = FormatData(data);
            return Post<T>(url, s, cc, refer);
        }

        public static HttpStatusCode HeadHttpCode(string url, string data = null, CookieContainer cc = null, string refer = null)
        {
            try
            {
                var r = WebRequest.Create(url) as HttpWebRequest;
                ...
                r.Referer = refer;
                r.CookieContainer = cc;
                return ((HttpWebResponse)r.GetResponse()).StatusCode;
            }
            catch
            {
        ...
        private static string FormatData(IEnumerable<KeyValuePair<string, object>> data)
        {
            var s = data == null
                ? null
                : string.Join("&", data.Select(d => string.Format("{0}={1}", d.Key, Uri.EscapeDataString(Convert.ToString(d.Value)))));
