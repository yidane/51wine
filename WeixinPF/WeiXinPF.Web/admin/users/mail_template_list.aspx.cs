    ...
    public partial class mail_template_list : Web.UI.ManagePage
    {
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("mail_template_page_size", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_mail_template", MXEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.mail_template bll = new BLL.mail_template();
            ...
            AddAdminLog(MXEnums.ActionEnum.Delete.ToString(), "删除邮件模板成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("mail_template_list.aspx", "keywords={0}", this.keywords), "Success");
        }
