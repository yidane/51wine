...
namespace WeiXinPF.Web.admin.dialog
{
    public partial class selectmodulelink : Web.UI.ManagePage
    {
        WeiXinPF.BLL.wx_link_module bll = new WeiXinPF.BLL.wx_link_module();
            ...
            { 
               ...
               if (ds != null && ds.Tables.Count > 0)
               {
                   ...
                   for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                   {
                       ...
                       tmpUrl = tmpUrl.Replace("{id}", id);
                       sb.Append(" <tr class=\"td_c\">  <td><label class=\"lbl_modulerad\"> <input id=\"rad_" + id + "\" class=\"s_modulerad\" type=\"radio\" name=\"modulerad\" /><label for=\"rad_" + id + "\" title=\"" + tmpUrl + "\" class=\"lblUrl\">" + name + "[编码：" + id + "]</label></label> </td>  </tr>");
