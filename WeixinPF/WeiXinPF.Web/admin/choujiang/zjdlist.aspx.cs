...
{
    public partial class zjdlist : Web.UI.ManagePage
    {
        protected int totalCount;
        protected int page;
        protected int pageSize;
        BLL.wx_zjdActionInfo gbll = new BLL.wx_zjdActionInfo();
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    dr["url"] = MyCommFun.getWebSite() + "/weixin/zjd/index.aspx?wid=" + dr["wid"].ToString() + "&aid=" + dr["id"].ToString();
                }
                ds.AcceptChanges();
            }
            ...
            string pageUrl = Utils.CombUrlTxt("zjdlist.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("zjdlist_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("zjdlist.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("zjdlist_page_size", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("zjdlist.aspx", "keywords={0}", this.keywords));
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("zjdlist.aspx", "keywords={0}", this.keywords), "Success");
        }
