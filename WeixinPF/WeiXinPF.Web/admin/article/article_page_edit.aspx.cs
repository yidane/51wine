...
{
    public partial class article_page_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
           ...
           // model.link_url = txtLinkUrl.Text.Trim();
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(MXEnums.ActionEnum.Add.ToString(), "添加单页" + this.channel_name + "内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
           ...
           // model.link_url = txtLinkUrl.Text.Trim();
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(MXEnums.ActionEnum.Edit.ToString(), "修改单页" + this.channel_name + "内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == MXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改单页信息成功！", "article_page_list.aspx?channel_id=" + this.channel_id, "Success");
            }
            else //添加
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", MXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加单页信息成功！", "article_page_list.aspx?channel_id=" + this.channel_id, "Success");
            }
        }
    }
}