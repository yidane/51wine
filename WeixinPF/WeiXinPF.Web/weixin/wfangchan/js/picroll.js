...
FCAPP.HOUSE.Picroll = {
    ...
    loadData: function () {
        ...
        $.ajax({
            url: '../wfangchan/cldata.ashx?myact=htImg&htid=' + htid + '&wid=' + wid + '&fid=' + fid,
            dataType: 'jsonp',
            error: function () {
    ...
    renderPics: function (room) {
        ...
        room.total = pics.length;//加载第一张图片下面显示的图片总数，markUpdate
        detail = $.template(R.template, {
            data: room
        });
        //markUpdate---------------------bimg不需要显示
        ...
        il = pics.length; i < il; i++) {
            List.push('<img id="pic' + (i) + '" idx="pic' + (i) + '" width="' + R.w + '" border="0" height="' + R.h + '" ' + imgdata + '>');
            R.Img.push(pics[i].img);
            R.ImgName.push([(i + 1) + '/' + (il), pics[i].name.replace(/\.jpe?g$/gi, ''), room.name.replace(/\.jpe?g$/gi, '')]);
            R.loadSize.push({
                w: pics[i].width,
                h: pics[i].height
            });
