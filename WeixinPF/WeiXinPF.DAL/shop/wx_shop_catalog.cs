...
namespace WeiXinPF.DAL
{
	...
	public partial class wx_shop_catalog
	{
		...
		public WeiXinPF.Model.wx_shop_catalog GetModel(int id)
		{
			...
			WeiXinPF.Model.wx_shop_catalog model=new WeiXinPF.Model.wx_shop_catalog();
		...
		/// 得到一个对象实体
		/// </summary>
		public WeiXinPF.Model.wx_shop_catalog DataRowToModel(DataRow row)
		{
			WeiXinPF.Model.wx_shop_catalog model=new WeiXinPF.Model.wx_shop_catalog();
			if (row != null)
			{
        ...
        public bool DeleteCataLog(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from wx_shop_catalog ");
            strSql.Append(" where id=@id  and id not in(select catalogid from wx_shop_catalog_attribute) and id not in(select isnull(catalogid,0) from wx_shop_product) ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
            parameters[0].Value = id;
