...
namespace WeiXinPF.DAL
{
	...
	public partial class wx_userweixin
	{
        ...
        //    WeiXinPF.Model.wx_userweixin model=new WeiXinPF.Model.wx_userweixin();
		...
		/// 得到一个对象实体
		/// </summary>
		public WeiXinPF.Model.wx_userweixin DataRowToModel(DataRow row)
		{
			WeiXinPF.Model.wx_userweixin model=new WeiXinPF.Model.wx_userweixin();
			if (row != null)
			{
...
m.user_name,m.real_name,m.is_lock from wx_userweixin w left join dt_manager m on w.uId=m.id where w.isDelete=0  ");
        ...
        public Model.wx_userweixin GetModel(int id)
        {
            ...
            WeiXinPF.Model.wx_userweixin model = model = new WeiXinPF.Model.wx_userweixin();
            while (sr.Read())
            {
                #region while循环
