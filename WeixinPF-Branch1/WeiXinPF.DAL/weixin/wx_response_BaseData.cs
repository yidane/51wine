...
namespace WeiXinPF.DAL
{
    ...
    public partial class wx_response_BaseData
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(WeiXinPF.Model.wx_response_BaseData model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(WeiXinPF.Model.wx_response_BaseData model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public WeiXinPF.Model.wx_response_BaseData GetModel(int id)
        {
            ...
            WeiXinPF.Model.wx_response_BaseData model = new WeiXinPF.Model.wx_response_BaseData();
        ...
        /// 得到一个对象实体
        /// </summary>
        public WeiXinPF.Model.wx_response_BaseData DataRowToModel(DataRow row)
        {
            WeiXinPF.Model.wx_response_BaseData model = new WeiXinPF.Model.wx_response_BaseData();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"select id, wid,nickname,headimgurl,  headerpic,  wx_openid, requestType, requestContent, responseType, reponseContent,           createTime, wx_xmlContent, createDate, flag, rType, remark, extInt, extStr, extStr2, extStr3 from
            (select r.id,headimgurl, r.wid,nickname, wx_openid, requestType, requestContent, responseType, reponseContent, createTime, wx_xmlContent,headerpic,          r.createDate, flag, rType, r.remark, r.extInt, r.extStr, r.extStr2, r.extStr3 
            from wx_response_BaseData r left join wx_crm_users u on r.wx_openid=u.openid left join wx_userweixin s on r.wid=s.id) a ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where  " + strWhere);
            }
