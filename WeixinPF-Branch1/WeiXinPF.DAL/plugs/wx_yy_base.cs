...
namespace WeiXinPF.DAL
{
	...
	public partial class wx_yy_base
	{
		...
		public int Add(WeiXinPF.Model.wx_yy_base model)
		{
            ...
            strSql.Append("wid,title,addr,phone,kfName,content,picUrl,beginDate,endDate,remark,extInt,extStr,extStr2,createDate,seq,needSMS,needPIC)");
			...
			SqlParameter[] parameters = {
                   ...
                   new SqlParameter("@needPIC", SqlDbType.Bit,1)};
            ...
            parameters[16].Value = model.needPIC;
		...
		public bool Update(WeiXinPF.Model.wx_yy_base model)
		{
            ...
            parameters[17].Value = model.needPIC;
		...
		public WeiXinPF.Model.wx_yy_base GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,wid,title,addr,phone,kfName,content,picUrl,beginDate,endDate,remark,extInt,extStr,extStr2,createDate,seq,needSMS,needPIC from wx_yy_base ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			WeiXinPF.Model.wx_yy_base model=new WeiXinPF.Model.wx_yy_base();
		...
		/// 得到一个对象实体
		/// </summary>
		public WeiXinPF.Model.wx_yy_base DataRowToModel(DataRow row)
		{
			WeiXinPF.Model.wx_yy_base model=new WeiXinPF.Model.wx_yy_base();
			if (row != null)
			{
                ...
                if (row["needPIC"] != null && row["needPIC"].ToString() != "")
                {
                    if ((row["needPIC"].ToString() == "1") || (row["needPIC"].ToString().ToLower() == "true"))
                    {
                        model.needPIC = true;
                    }
                    else
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select id,wid,title,addr,phone,kfName,content,picUrl,beginDate,endDate,remark,extInt,extStr,extStr2,createDate,seq,needSMS,needPIC");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" id,wid,title,addr,phone,kfName,content,picUrl,beginDate,endDate,remark,extInt,extStr,extStr2,createDate,seq,needSMS,needPIC");
			strSql.Append(" FROM wx_yy_base ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
