...
namespace WeiXinPF.DAL
{
	...
	public partial class wx_wq_sygj
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("Id", "wx_wq_sygj"); 
		}
		...
		public int Add(WeiXinPF.Model.wx_wq_sygj model)
		{
			...
			strSql.Append("url,Name,sort_id,createdate,gjstatus,wid)");
			...
			parameters[4].Value = model.gjstatus;
		...
		public WeiXinPF.Model.wx_wq_sygj GetModel(int Id)
		{
			...
			WeiXinPF.Model.wx_wq_sygj model=new WeiXinPF.Model.wx_wq_sygj();
		...
		/// 得到一个对象实体
		/// </summary>
		public WeiXinPF.Model.wx_wq_sygj DataRowToModel(DataRow row)
		{
			WeiXinPF.Model.wx_wq_sygj model=new WeiXinPF.Model.wx_wq_sygj();
			if (row != null)
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select a.* from wx_wq_sygj a ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where  " + strWhere);
            }
