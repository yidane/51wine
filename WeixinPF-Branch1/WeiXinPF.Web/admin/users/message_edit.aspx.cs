...
namespace WeiXinPF.Web.admin.users
{
    public partial class message_edit : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("user_message", MXEnums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
                JscriptMsg("发送过程中发生错误！", "", "Error");
                return;
            }
            AddAdminLog(MXEnums.ActionEnum.Add.ToString(), "发送站内短消息"); //记录日志
            JscriptMsg("发送短消息成功", "message_list.aspx", "Success");
        }
    }
}