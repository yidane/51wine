...
namespace WeiXinPF.Web.shopmgr.setting
{
    ...
    {
        WeiXinPF.BLL.wx_shop_setting bll = new WeiXinPF.BLL.wx_shop_setting();
        ...
        private void ShowInfo()
        {
            ...
            lblDemain.Text = MyCommFun.getWebSite().Replace("http://","");

            IList<WeiXinPF.Model.wx_shop_setting> modellist = bll.GetModelList("wId=" + weixin.id);
            if (modellist == null || modellist.Count <= 0)
            {
                return;
            }
            WeiXinPF.Model.wx_shop_setting model = modellist[0];
        ...
        {
            WeiXinPF.Model.wx_shop_setting model = new WeiXinPF.Model.wx_shop_setting();
            try
            {
                Model.wx_userweixin weixin = GetWeiXinCode();
                int wId = weixin.id;
