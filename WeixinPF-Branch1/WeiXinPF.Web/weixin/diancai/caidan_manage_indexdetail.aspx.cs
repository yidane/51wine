...
namespace WeiXinPF.Web.weixin.diancai
{
    public partial class caidan_manage_indexdetail : WeiXinPage
    {
        ...
        public void List(string id, int shopid)
        {



            DataSet deskdata = deskbll.GetListdesk(shopid);
            if (deskdata.Tables[0].Rows.Count>0)
            {
                for (int i = 0; i < deskdata.Tables[0].Rows.Count;i++ )
                {
                    option += " <option  value=" + deskdata.Tables[0].Rows[i]["deskName"].ToString() + ">" + deskdata.Tables[0].Rows[i]["deskName"].ToString() + "</option>";
                }
            }
            ...
            if (managemodel != null)
            {
                ...
                else if (managemodel.payStatus.ToString() == "2")
                {
                    dingdandatail += "<em  style='width:70px;' class='error'>失败</em></span></li>";
                }
                else
                {
