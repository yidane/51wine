...
{
	/// <summary>
	/// 数据访问类:wx_zjdAwardUser
	/// </summary>
	public partial class wx_zjdAwardUser
	{
		public wx_zjdAwardUser()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "wx_zjdAwardUser"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from wx_zjdAwardUser");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(MxWeiXinPF.Model.wx_zjdAwardUser model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into wx_zjdAwardUser(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(MxWeiXinPF.Model.wx_zjdAwardUser model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update wx_zjdAwardUser set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from wx_zjdAwardUser ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public MxWeiXinPF.Model.wx_zjdAwardUser GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,actId,uName,uTel,openid,jxName,jxIndex,jpName,createDate,hasLingQu,sn from wx_zjdAwardUser ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			MxWeiXinPF.Model.wx_zjdAwardUser model=new MxWeiXinPF.Model.wx_zjdAwardUser();
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM wx_zjdAwardUser ");
			if(strWhere.Trim()!="")
			{
        ...
        public MxWeiXinPF.Model.wx_zjdAwardUser DataRowToModel(DataRow row)
        {
            MxWeiXinPF.Model.wx_zjdAwardUser model = new MxWeiXinPF.Model.wx_zjdAwardUser();
            if (row != null)
            {
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from wx_zjdAwardUser T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "wx_zjdAwardUser";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update wx_zjdAwardUser set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        public DataSet GetListnum(string openid)
        {
            ...
            strSql.Append(" FROM wx_zjdAwardUser where openid='" + openid + "' ");
           
            return DbHelperSQL.Query(strSql.ToString());
        }


        public DataSet GetListhj(int aid,string openid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select u.*,i.jiangpinpic  FROM wx_zjdAwardUser u ");
            strSql.Append(" left join wx_zjdAwardItem  i on u.actId=i.actId and u.jxIndex=i.sort_id  where  u.openid='" + openid + "' and u.actId='"+aid+"'  ");
        ...
        public DataSet GetList(int aid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,actId,uName,uTel,openid,jxName,jxIndex,jpName,createDate,hasLingQu,sn ");
            strSql.Append(" FROM wx_zjdAwardUser  where actId="+aid+"  order by createDate desc,id desc ");

            return DbHelperSQL.Query(strSql.ToString());
        }


        public MxWeiXinPF.Model.wx_zjdAwardUser getZJinfoByOpenid(int actId, string openid)
        {
            ...
            MxWeiXinPF.Model.wx_dzpAwardUser model = new MxWeiXinPF.Model.wx_dzpAwardUser();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
        ...
        public DataSet getHasZJList(int actId)
        {
            ...
            strSql.Append(" FROM wx_zjdAwardUser where actId=@actId");
