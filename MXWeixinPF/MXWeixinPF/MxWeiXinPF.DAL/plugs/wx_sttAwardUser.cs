...
namespace MxWeiXinPF.DAL
{
	...
	public partial class wx_sttAwardUser
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "wx_sttAwardUser"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from wx_sttAwardUser");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(MxWeiXinPF.Model.wx_sttAwardUser model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into wx_sttAwardUser(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(MxWeiXinPF.Model.wx_sttAwardUser model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update wx_sttAwardUser set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from wx_sttAwardUser ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public MxWeiXinPF.Model.wx_sttAwardUser GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,actId,uName,uTel,openid,jxName,jpName,createDate,hasLingQu,sn from wx_sttAwardUser ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			MxWeiXinPF.Model.wx_sttAwardUser model=new MxWeiXinPF.Model.wx_sttAwardUser();
		...
		/// 得到一个对象实体
		/// </summary>
		public MxWeiXinPF.Model.wx_sttAwardUser DataRowToModel(DataRow row)
		{
			MxWeiXinPF.Model.wx_sttAwardUser model=new MxWeiXinPF.Model.wx_sttAwardUser();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM wx_sttAwardUser ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from wx_sttAwardUser T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "wx_sttAwardUser";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  *   from wx_sttAwardUser   ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where  " + strWhere);
            }
        ...
        public DataSet getHasZJList(int actId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,actId,uName,uTel,openid,jxName,jpName,createDate,hasLingQu,sn ");
            strSql.Append(" FROM wx_sttAwardUser where actId=@actId");
        ...
        /// 该用户的中奖信息
        /// </summary>
        public MxWeiXinPF.Model.wx_sttAwardUser getZJinfoByOpenid(int actId, string openid)
        {
