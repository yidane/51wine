...
namespace MxWeiXinPF.BLL
{
    public class url_rewrite
    {
        ...
        {
            List<Model.url_rewrite> ls = CacheHelper.Get<List<Model.url_rewrite>>(MXKeys.CACHE_SITE_URLS_LIST);
            if (ls == null)
            {
                CacheHelper.Insert(MXKeys.CACHE_SITE_URLS_LIST, dal.GetList(""), Utils.GetXmlMapPath(MXKeys.FILE_URL_XML_CONFING));
                ls = CacheHelper.Get<List<Model.url_rewrite>>(MXKeys.CACHE_SITE_URLS_LIST);
            }
            return ls;
        }
