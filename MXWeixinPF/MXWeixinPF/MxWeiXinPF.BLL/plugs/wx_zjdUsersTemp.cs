...
namespace MxWeiXinPF.BLL
{
	...
	{
		private readonly MxWeiXinPF.DAL.wx_zjdUsersTemp dal=new MxWeiXinPF.DAL.wx_zjdUsersTemp();
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_zjdUsersTemp> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_zjdUsersTemp> DataTableToList(DataTable dt)
		{
			List<MxWeiXinPF.Model.wx_zjdUsersTemp> modelList = new List<MxWeiXinPF.Model.wx_zjdUsersTemp>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MxWeiXinPF.Model.wx_zjdUsersTemp model;
