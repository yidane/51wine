...
namespace MxWeiXinPF.BLL
{
	...
	{
		private readonly MxWeiXinPF.DAL.wx_ggkUsersTemp dal=new MxWeiXinPF.DAL.wx_ggkUsersTemp();
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_ggkUsersTemp> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_ggkUsersTemp> DataTableToList(DataTable dt)
		{
			List<MxWeiXinPF.Model.wx_ggkUsersTemp> modelList = new List<MxWeiXinPF.Model.wx_ggkUsersTemp>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MxWeiXinPF.Model.wx_ggkUsersTemp model;
