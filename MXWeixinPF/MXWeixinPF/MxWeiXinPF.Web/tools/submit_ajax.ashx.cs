...
namespace MxWeiXinPF.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(MXKeys.COOKIE_USER_NAME_REMEMBER, "MxWeiXinPF", model.user_name, 43200);
                Utils.WriteCookie(MXKeys.COOKIE_USER_PWD_REMEMBER, "MxWeiXinPF", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = MxWeiXinPF.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = MxWeiXinPF.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            ...
            //清空购物车
            MxWeiXinPF.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
