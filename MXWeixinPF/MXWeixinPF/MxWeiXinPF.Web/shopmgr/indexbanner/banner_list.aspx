<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="banner_list.aspx.cs" Inherits="MxWeiXinPF.Web.shopmgr.indexbanner.banner_list" %>
...
<body class="mainbody">
    <form id="form1" runat="server">
        <!--导航栏-->
        <div class="location">
           <a class="home"><i></i><span>图片管理</span></a>
        </div>
        <!--/导航栏-->
        ...
        <div class="toolbar-wrap">
            <div id="floatHead" class="toolbar">
                <div class="l-list">
                    <ul class="icon-list">
                        <li><a class="icon-btn add" href="banner_edit.aspx?action=<%=MXEnums.ActionEnum.Add %>&" id="itemAddButton"><i></i><span>新增图片</span></a></li>
        ...
        <!--列表-->
         ...
         <asp:Repeater ID="rptList2" runat="server" OnItemCommand="rptList_ItemCommand">
            ...
            <ItemTemplate>
                <li>
                    <div class="details<%#Eval("bannerPicUrl").ToString() != "" ? "" : " nopic"%>">
                        <div class="check">
                            <asp:CheckBox ID="chkId" CssClass="checkall" runat="server" /><asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" />
                        </div>
                        <%#Eval("bannerPicUrl").ToString() != "" ? "<div class=\"pic\"><img src=\"../images/loadimg.gif\" data-original=\"" + Eval("bannerPicUrl") + "\" /></div><i class=\"absbg\"></i>" : ""%>
                        <h1><span><%#Eval("bannerName")%></span></h1>
                        <div class="remark" style=" height:20px;">
                            <%#Eval("remark").ToString() == "" ? "暂无描述说明..." : Eval("remark").ToString()%>
                        </div>
                        <div class="tools">

                           <asp:Label ID="Label1" runat="server" Text='<%#Eval("sort_id")%>' style="float:right;"></asp:Label>
                        ...
                        <div class="foot">
                            ...
                            <a href="banner_edit.aspx?action=<%#MXEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
                        </div>
                    </div>
                </li>
            </ItemTemplate>
