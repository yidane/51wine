...
namespace MxWeiXinPF.Web.admin.order
{
    public partial class order_list : Web.UI.ManagePage
    {
        ...
        {
            this.status = MXRequest.GetQueryInt("status");
            this.payment_status = MXRequest.GetQueryInt("payment_status");
            this.express_status = MXRequest.GetQueryInt("express_status");
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind(" " + CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords), "status asc,add_time desc,id desc");
            }
        }
        ...
        {
            ChkAdminLevel("order_list", MXEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
            ...
            AddAdminLog(MXEnums.ActionEnum.Delete.ToString(), "删除订单成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
