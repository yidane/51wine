...
{
    public partial class caipin_manage : Web.UI.ManagePage
    {
        ...
        protected string CombSqlTxt(string _keywords)
        {
            ...
            {
                strTemp.Append(" and  cpName like  '%" + _keywords + "%' ");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("caipin_manage_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("caipin_manage.aspx", "keywords={0}&shopid={1}", txtKeywords.Text, shopid.ToString()));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("caipin_manage_page_size", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("caipin_manage.aspx", "keywords={0}&shopid={1}", this.keywords, shopid.ToString()));
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("caipin_manage.aspx", "keywords={0}&shopid={1}", this.keywords, shopid.ToString()), "Success");
        }
