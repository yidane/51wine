<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="index.aspx.cs" Inherits="MxWeiXinPF.Web.weixin.cards.index" %>
<html>
<head>
        ...
        document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
            ...
            WeixinJSBridge.on('menu:share:appmessage', function (argv) {
                WeixinJSBridge.invoke('sendAppMessage', {
                    ...
                    "img_width": "640",
                    "img_height": "640",
                    "link": "http://" + window.location.host + "/weixin/cards/index.aspx?aid=<%=id%>&wid=<%=wid%>&dh=<%=dhId%>&name=" + encodeURIComponent($.trim($(".name").html())) + "&characters=" + encodeURIComponent($.trim($(".message").html())) + "&from=app",
                    "desc": $(".message").html(),
                    "title": "<%=title %>"
                }, function (res) {
                    var ret = zfj1();
            ...
            WeixinJSBridge.on('menu:share:weibo', function (argv) {
                WeixinJSBridge.invoke('shareWeibo', {
                    ...
                    "url": "http://" + window.location.host + "/weixin/cards/index.aspx?aid=<%=id%>&wid=<%=wid%>&dh=<%=dhId%>&name=" + encodeURIComponent($.trim($(".name").html())) + "&characters=" + encodeURIComponent($.trim($(".message").html())) + "&from=app"
                }, function (res) {
                    var ret = zfj1();
                    var result = syncEvent(ret);
