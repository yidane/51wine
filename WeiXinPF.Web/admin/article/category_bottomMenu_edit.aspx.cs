...
{
    public partial class category_bottomMenu_edit : Web.UI.ManagePage
    {
        private string action = MXEnums.ActionEnum.Add.ToString(); //操作类型
        protected string channel_name = "bottommenu"; //频道名称
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == MXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改类别成功！", "category_bottomMenu_list.aspx?channel_id=" + channel_id, "Success");
            }
            else //添加
            {
                ChkAdminLevel("bottomnavigation", MXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
                }
                JscriptMsg("添加类别成功！", "category_bottomMenu_list.aspx?channel_id=" + channel_id, "Success");
            }
        }
    }
}