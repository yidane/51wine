...
{
    public partial class wx_article_edit : Web.UI.ManagePage
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == MXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改信息成功！", "wx_article_list.aspx?channel_id=" + this.channel_id, "Success");
            }
            else //添加
            {
                ChkAdminLevel("articlemgr", MXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加信息成功！", "wx_article_list.aspx?channel_id=" + this.channel_id, "Success");
            }
        }
    }
}