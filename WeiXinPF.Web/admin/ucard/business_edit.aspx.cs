...
namespace WeiXinPF.Web.admin.ucard
{
    public partial class business_edit : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("ucard_store", MXEnums.ActionEnum.View.ToString()); //检查权限
            int id = 0;
        ...
        {
            Model.wx_userweixin weixin = GetWeiXinCode();
            litUrl.Text = MyCommFun.getWebSite() + "/weixin/ucard/index.aspx?wid=" + weixin.id;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (id <= 0)
            {  //新增
                ChkAdminLevel("ucard_store", MXEnums.ActionEnum.Add.ToString()); //检查权限
                store.wid = weixin.id;
                ...
                JscriptMsg("添加会员卡商家成功！", "business_edit.aspx", "Success");
            }
            else
            {   //修改
                ChkAdminLevel("ucard_store", MXEnums.ActionEnum.Edit.ToString()); //检查权限
                //1修改主表
                ...
                JscriptMsg("修改会员卡商家成功！", "business_edit.aspx", "Success");
            }
