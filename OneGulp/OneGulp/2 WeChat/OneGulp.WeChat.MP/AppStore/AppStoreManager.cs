...
using OneGulp.WeChat.MP.AppStore.Api;
...
namespace OneGulp.WeChat.MP.AppStore
{
    public class AppStoreManager
    {
        ...
        public static void ApplyPassport(string appKey, string appSecret, string url = DEFAULT_URL)
        {
            ...
            var result = OneGulp.WeChat.HttpUtility.Post.PostGetJson<PassportResult>(getPassportUrl, formData: formData);
            if (result.Result != AppResultKind.成功)
            {
                throw new WeChatException("获取Passort失败！错误信息：" + result.Result, null);
            }
