...
using OneGulp.WeChat.MP.AppStore;
...
namespace OneGulp.WeChat.MP.MessageHandlers
{
    public interface IMessageHandler : WeChat.MessageHandlers.IMessageHandler<IRequestMessageBase, IResponseMessageBase>
    {
    ...
    /// <summary>
    /// 微信请求的集中处理方法
    /// 此方法中所有过程，都基于OneGulp.WeChat.MP的基础功能，只为简化代码而设。
    /// </summary>
    public abstract class MessageHandler<TC> :
        WeChat.MessageHandlers.MessageHandler<TC, IRequestMessageBase, IResponseMessageBase>, IMessageHandler
        where TC : class, IMessageContext<IRequestMessageBase, IResponseMessageBase>, new()
    {
        /// <summary>
        /// 上下文（仅限于当前MessageHandler基类内）
        /// </summary>
        public static WeChatContext<TC, IRequestMessageBase, IResponseMessageBase> GlobalWeChatContext = new WeChatContext<TC, IRequestMessageBase, IResponseMessageBase>();
        //TODO:这里如果用一个MP自定义的WeChatContext，继承WeChatContext<TC, IRequestMessageBase, IResponseMessageBase>，在下面的WeChatContext中将无法转换成基类要求的类型


        /// <summary>
        /// 全局消息上下文
        /// </summary>
        public override WeChatContext<TC, IRequestMessageBase, IResponseMessageBase> WeChatContext
        {
            get
            {
                return GlobalWeChatContext;
            }
        }
        ...
        /// 这里是OneGulp.WeChat.MP，根据请求消息半段在ResponseDocument基础上是否需要再次进行加密（每次获取重新加密，所以结果会不同）
        /// </summary>
        ...
        {
            DeveloperInfo = developerInfo;
            //WeChatContext.MaxRecordCount = maxRecordCount;
        ...
        public override XDocument Init(XDocument postDataDocument, object postData = null)
        {
            ...
            //记录上下文
            if (WeChatContextGlobal.UseWeChatContext)
            {
                WeChatContext.InsertMessage(RequestMessage);
            }
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                //记录上下文
                if (WeChatContextGlobal.UseWeChatContext && ResponseMessage != null)
                {
                    WeChatContext.InsertMessage(ResponseMessage);
                }
            }
            catch (Exception ex)
            {
        ...
        public virtual IResponseMessageBase OnEventRequest(IRequestMessageEventBase requestMessage)
        {
            ...
            switch (strongRequestMessage.Event)
            {
                ...
                case Event.location_select://弹出地理位置选择器
                    ...
                    break;
                case Event.pic_WeChat://弹出微信相册发图器
                    responseMessage = OnEvent_PicWeChatRequest(RequestMessage as RequestMessageEvent_Pic_WeChat);
                    break;
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IResponseMessageBase OnEvent_PicWeChatRequest(RequestMessageEvent_Pic_WeChat requestMessage)
        {
            return DefaultResponseMessage(requestMessage);
        }
