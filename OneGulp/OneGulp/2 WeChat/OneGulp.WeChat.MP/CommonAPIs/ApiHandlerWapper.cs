...
using OneGulp.WeChat.Utilities.WeChatUtility;

namespace OneGulp.WeChat.MP
{
    /// <summary>
    ...
    public static class ApiHandlerWapper
    {
        ...
        public static T TryCommonApi<T>(Func<string, T> fun, string accessTokenOrAppId = null, bool retryIfFaild = true) where T : WxJsonResult
        {
            ...
            if (accessTokenOrAppId == null)
            {
                ...
                if (appId == null)
                {
                    throw new WeChatException("尚无已经注册的AppId，请先使用AccessTokenContainer.Register完成注册（全局执行一次即可）！");
                }
            }
            else if (ApiUtility.IsAppId(accessTokenOrAppId))
            {
                ...
                {
                    throw new WeChatException("此appId尚未注册，请先使用AccessTokenContainer.Register完成注册（全局执行一次即可）！");
                }
